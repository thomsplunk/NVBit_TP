...
// Set this to 0 to use more than 64 Kb of shared memory to cache data, to  improve the performance of the computations on GPU.
...
// With only 64 Kb shared memory available, we can fit two 8-tile chunks of the A and B matrix data, that are 16 * 16 * 8 * 8 * 2 = 32 Kb each (i.e. two 8x8 arrays of tiles of 16x16 half-typed elements per CTA).
// But we cannot account the 8 Kb total skew overhead, without which the performance would be severely impacted. So we choose to reduce the chunk size in half, i.e. the amount of A and B matrix data we cache in shared memory.
// Accordingly, this doubles the number of outer iterations across the global K dimension, which only slightly impacts the performance.
#define CHUNK_K 4
#else
#define CHUNK_K 8
#endif
...
__host__ void init_host_matrices(half *a, half *b, float *c) {
  for (int i = 0; i < M_GLOBAL; i++) {
    for (int j = 0; j < K_GLOBAL; j++) {
      a[i * K_GLOBAL + j] = __float2half(2.0f);
    }
  }

  for (int i = 0; i < N_GLOBAL; i++) {
    for (int j = 0; j < K_GLOBAL; j++) {
      b[i * K_GLOBAL + j] = __float2half(2.0f);
    }
  }
...
printf("Output Matrix D:\n");
float *D_h = (float *)malloc(sizeof(float) * M_GLOBAL * N_GLOBAL);
checkCudaErrors(cudaMemcpy(D_h, D, sizeof(float) * M_GLOBAL * N_GLOBAL, cudaMemcpyDeviceToHost));

for (int i = 0; i < M_GLOBAL; ++i) {
    for (int j = 0; j < N_GLOBAL; ++j) {
        printf("%.2f ", D_h[i * N_GLOBAL + j]);
    }
    printf("\n");
}
